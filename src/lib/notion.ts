...
export async function getChildPages(pageId: string): Promise<NotionPage[]> {
  ...
  // Convert child_page blocks to page objects using block information directly
  const childPages: NotionPage[] = childPageBlocks.map((block) => {
    ...
    // Create a minimal page object from block information
    return {
      id: block.id,
